...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("Status Serwera OwnRP", "https://cdn.discordapp.com/attachments/763426371799613494/807976241863983114/OwnRolePlay.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ OwnRP' : '⚫ OwnRP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nJak dołączyć na serwer?','To proste! Klikając **F8** wpisujesz `connect 176.9.136.93:31714`. Oczywiście musisz posiadać role `@「💫」WhiteList`, aby wejść na serwer. \n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  const offline = function() {
    ...
    .setColor(0xff0000)
    .addField('Status Serwera',':x: Offline',true)
    .addField('Gracze w kolejce','?',true)
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('Status Serwera',':white_check_mark: Online',true)
        .addField('Gracze w Kolejce',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('OwnRP',{'url':'https://www.twitch.tv/wionzu','type':'STREAMING'});
    ...
    if (!message.author.bot) {
      if (message.member) {
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setColor(0x2894C2)
          .setTitle('Bug Report')
          .setDescription('Bug został zgłoszony!')
          .setTimestamp(new Date());
